...
Run on `gemini-2.0-flash` — the same model family used for most of VERITAS's own nodes, so the comparison measures the *workflow*, not a model swap. See [benchmark/run_baseline.py](../benchmark/run_baseline.py).
